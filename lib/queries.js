const queries = {
    getTable: function getTable(table) {
        ...
        switch (table) {
            case 'employees':
                query = `SELECT first_name, last_name, title, salary, name AS 'department' FROM employees JOIN roles ON employees.role_id = roles.id JOIN departments ON employees.role_id = departments.id`;
                break;
            case 'roles':
