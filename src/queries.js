const queries = {
    getTable: function getTable(table) {
        ...
        switch (table) {
            case 'employees':
                query = `SELECT employees.id, employees.first_name, employees.last_name, roles.title, roles.salary, departments.name AS 'department', CONCAT(managers.first_name, ' ', managers.last_name) AS 'managers name' FROM employees LEFT JOIN roles ON employees.role_id = roles.id LEFT JOIN departments ON roles.department_id = departments.id LEFT JOIN employees managers ON employees.manager_id = managers.id`;
                break;
            case 'roles':
                query = `SELECT roles.id, title, salary, name AS 'department' FROM roles JOIN departments ON roles.department_id = departments.id`;
                break;
            case 'departments':
                query = `SELECT id, name as 'department name' FROM departments`;
                break;
        }
        return query;
    },
    ...
    getManagers: function getManagers() {
        return `SELECT employees.id, employees.first_name, employees.last_name FROM employees JOIN roles ON employees.role_id = roles.id WHERE title = 'manager'`;
    },
    ...
    updateRow: function updateRow(table, field) {
        switch (table) {
            case "employees":
                switch (field) {
                    case "role":
